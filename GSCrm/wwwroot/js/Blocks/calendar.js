class Calendar {
    ...
    Initialize(field, e) {
        ...
        $('.select-area')
            ...
            .on('click', '.month-day', event => {
                ...
                $('#' + $('.select-area').attr('data-target-id'))
                    .find('.gs-field-input')
    ...
    SetStyle() {
        var date = new Date();
        $('.select-area').attr('data-target-id', $(this.Field).attr('id'));
    ...
    SetLocation(event) {
        ...
        $('.select-area').attr('style', '');

        // Если календарь открыт в модальном окне
        if ($(event.currentTarget).closest('.all-popup').length !== 0) {

            var calendarHeight = $('.select-area').find('.calendar-mini').height();     // Высота выпадушки
            ...
            var modalTop = $('.p-modal-head').offset().top;                             // Отступ заголовка модального окна сверху относительно документа
            var modalLeft = $('.p-modal-body').offset().left;                           // Отступ заголовка модального окна слева относительно документа
            ...
            if (fieldModalRight < 400) {
                ...
                $('.select-area')[0].style.left = (modalLeft + fieldModalLeft + fieldWidth - 400) + 'px';
            }

            // Если слева от поля в модальном окне нет места для календаря
            if (fieldModalLeft < 400) {
                $('.select-area')[0].style.left = fieldLeft + 'px';
            }
            ...
            if ((fieldModalBottom < calendarHeight) && (fieldModalTop < (calendarHeight + fieldHeight))) {
                ...
                // Если слева от поля в модальном окне нет места для календаря
                if (fieldModalLeft < 400) {
                    $('.select-area')[0].style.left = (fieldLeft + fieldWidth) + 'px';
                }

                // Если справа от поля в модальном окне нет места для календаря
                else {
                    $('.select-area')[0].style.left = (fieldLeft - 400) + 'px';
                }
            }

            // Если ни слева ни справа от поля в модальном окне нет места для календаря
            if ((fieldModalRight < 400) && (fieldModalLeft < 400)) {
                $('.select-area')[0].style.left = ((windowWidth - 400) / 2) + 'px';
            }

            $('.select-area')[0].style.top = fieldTop + 'px';
            $('.select-area')[0].style.position = 'fixed';
        }
            ...
            if ((documentWidth - 400) < fieldLeft) {
                ...
                $('.select-area')[0].style.right = documentWidth - (fieldLeft + this.Field.offsetWidth) + 'px';
            }
